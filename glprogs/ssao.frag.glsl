...
float depthAt(vec2 texCoord) {
	return 2 * texture(depthTexture, texCoord).r - 1;
...
void main() {
	...
	float occlusion = 0.0;
	float radiusOverDepth = radius / (0.5*depth+0.5);
	for (int i = 0; i < samples; i++) {
		vec3 ray = radiusOverDepth * sample_sphere[i];
